 ...
 *
 * modified by Leonardo Cigolini Gulesu
 */
...
static void exc3000_report_slots(struct i2c_client *client,
				 struct input_dev *input,
				 struct touchscreen_properties *prop,
				 const u8 *buf, int num)
{
	...
	for (; num--; buf += EXC3000_LEN_POINT) {
		...
		if (down) {
			x = (unsigned long)get_unaligned_le16(buf + 2);
			y = (unsigned long)get_unaligned_le16(buf + 4);
			vx = (unsigned int)(x/4);
			vy = (unsigned int)((y*75)/512);
			touchscreen_report_pos(input, prop, vx, vy, true);
#ifdef REPORT_TOUCH
			sprintf(obuf, "ts report pos: x %d y %d\n", vx, vy);
			dev_err(&client->dev, obuf);
#endif
		}
	}
}

static void exc3000_start(struct exc3000_data *data)
{
	data->running = true;
        enable_irq(data->client->irq);
}

static void exc3000_stop(struct exc3000_data *data)
{
	data->running = false;
        disable_irq(data->client->irq);
}

static int exc3000_open(struct input_dev *dev)
{
	struct exc3000_data  *data = input_get_drvdata(dev);
	exc3000_start(data);
	return 0;
}

static void exc3000_close(struct input_dev *dev)
{
	struct exc3000_data *data = input_get_drvdata(dev);
	exc3000_stop(data);
}
...
static void exc3000_timer(unsigned long devAddr)
{
/*	struct exc3000_data *data = from_timer(data, t, timer); */
	struct exc3000_data *data = (struct exc3000_data *)devAddr;
...
static irqreturn_t exc3000_interrupt(int irq, void *dev_id)
{
	...
	if (error) {
		/* dev_err(&client->dev, "exc3000_irq error %d\n",error); */
	...
	while (total_slots > 0) {
		...
		exc3000_report_slots(data->client, input, &data->prop, buf + 4, slots);
		total_slots -= slots;
			 ...
			 const struct i2c_device_id *id)
{
	...
	input->open = exc3000_open;
	input->close = exc3000_close;
	...
	dev_err(&client->dev, "exc3000_probe completed\n");
	return 0;
}
